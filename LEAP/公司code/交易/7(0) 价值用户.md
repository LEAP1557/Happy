...
use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_bt_ord_dtl_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_bt_ord_dtl_tyd as
...
use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_bt_userpin_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_bt_userpin_tyd as
...
left join dmc_dev.btjzyh_bt_ord_dtl_tyd b on a.pin=b.user_pin and b.rn=1
;

use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_bt_userpin1_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_bt_userpin1_tyd as
select t1.*,t2.amt_fstdt,t2.amt_fstdt_jq_cmplt,t3.secdt_time,t3.secdt_hlth_flag,t3.secdt_loan_user_age
from dmc_dev.btjzyh_bt_userpin_tyd t1 
left join
(
...
from dmc_dev.btjzyh_bt_userpin_tyd a inner join
dmc_dev.btjzyh_bt_ord_dtl_tyd b on a.pin=b.user_pin and b.rn_dt=1
group by 1
) t2 on t1.pin=t2.pin
left join
(select user_pin,min(loan_time) secdt_time,min(hlth_flag) secdt_hlth_flag,min(loan_user_age) secdt_loan_user_age from dmc_dev.btjzyh_bt_ord_dtl_tyd where rn_dt=2 group by 1) t3 on t1.pin=t3.user_pin
;

use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_bt_userpin_1yrnew_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_bt_userpin_1yrnew_tyd as
select a.*,b.*
from dmc_dev.btjzyh_bt_ord_dtl_tyd a 
inner join 
dmc_dev.btjzyh_bt_userpin_tyd b on a.user_pin=b.pin and a.time_last>'1900-01-01 00:00:00' and datediff(a.loan_time,a.time_last)>=365
;

use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_bt_userpin2_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_bt_userpin2_tyd as
select a.*
,case 
...
from dmc_dev.btjzyh_bt_userpin1_tyd a
...
use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_bt_userpin_1yrnew_1_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_bt_userpin_1yrnew_1_tyd as
select a.*
,
case
...
from dmc_dev.btjzyh_bt_userpin_1yrnew_tyd a 
...
use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_value_userpin_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_value_userpin_tyd as
...
from (select * from dmc_dev.btjzyh_bt_userpin2_tyd where value_tm is not null) a 
full outer join
dmc_dev.btjzyh_bt_userpin_1yrnew_1_tyd b 
...
use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_value_userpin_cnt_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_value_userpin_cnt_tyd as
...
from dmc_dev.btjzyh_value_userpin_tyd
where substr(value_tm,1,10) between '2020-01-01' and '2021-08-26' and value_hlth_flag=1 and coalesce(value_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
...
from dmc_dev.btjzyh_value_userpin_tyd
where substr(value_tm,1,10) between '2020-01-01' and '2021-08-26' and chnl_cate_1 is not null and value_hlth_flag=1 and coalesce(value_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
...
from dmc_dev.btjzyh_value_userpin_cnt_tyd where grouping__id=1;
...
from dmc_dev.btjzyh_value_userpin_cnt_tyd where grouping__id=0 and chnl_cate_0 is not null and dt between '2021-08-01' and '2020-08-26';


use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btjzyh_value_userpin_cnt_online_tyd;
CREATE TABLE IF NOT EXISTS dmc_dev.btjzyh_value_userpin_cnt_online_tyd as
...
,valueuser2--价值用户-2类-新达成-首贷当天结清但后续复购
,users_jz-valueuser1-valueuser2 valueuser3_1 --价值用户-年新-剔除新达成2类重合
from dmc_dev.btjzyh_value_userpin_cnt_online_tyd where grouping__id=0 and chnl_online is not null and dt between '2021-09-01' and '2021-09-08';
