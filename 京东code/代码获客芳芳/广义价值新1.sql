...
left join idm.idm_f02_cf_xbt_loan_flag_i_d c on a.loan_id=c.loan_id and a.ordr_id=c.ordr_id
...
where substr(value_tm,1,10) between '2020-01-01' and '2021-09-28' and value_hlth_flag=1 and coalesce(value_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
...
from dmc_dev.btgyx_value_userpin
where substr(value_tm,1,10) between '2020-01-01' and '2021-09-28' and chnl_cate_1 is not null and value_hlth_flag=1 and coalesce(value_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
...
set hive.auto.convert.join=false;
use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btgyx_fst_userpin_cnt;
CREATE TABLE IF NOT EXISTS dmc_dev.btgyx_fst_userpin_cnt as
select t1.grouping__id,t1.dt,t1.chnl_cate,t1.users_fst,t2.users_fst users_fst_1
from
(select grouping__id, substr(fst_time,1,10) dt,chnl_cate,count(pin) users_fst
from dmc_dev.btgyx_bt_userpin2
where substr(fst_time,1,10) between '2020-01-01' and '2021-09-28' and fst_hlth_flag=1 and coalesce(fst_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
group by substr(fst_time,1,10),chnl_cate
grouping sets(substr(fst_time,1,10),(substr(fst_time,1,10),chnl_cate))
) t1
left join
(select substr(fst_time,1,10) dt,chnl_cate_1,count(pin) users_fst
from dmc_dev.btgyx_bt_userpin2
where substr(fst_time,1,10) between '2020-01-01' and '2021-09-28' and chnl_cate_1 is not null and fst_hlth_flag=1 and coalesce(fst_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
group by substr(fst_time,1,10),chnl_cate_1
) t2
on t1.dt=t2.dt and t1.grouping__id=0 and t1.chnl_cate=t2.chnl_cate_1
;

use dmc_dev;
DROP TABLE IF EXISTS dmc_dev.btgyx_fst_userpin_cnt_online;
CREATE TABLE IF NOT EXISTS dmc_dev.btgyx_fst_userpin_cnt_online as
select substr(fst_time,1,10) dt,chnl_online,count(pin) users_fst
from dmc_dev.btgyx_bt_userpin2
where substr(fst_time,1,10) between '2020-01-01' and '2021-09-28' and fst_hlth_flag=1 and coalesce(fst_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
group by 1,2
;
...
from dmc_dev.btgyx_value_userpin_cnt where grouping__id=0 and chnl_cate_0 is not null and dt = '2021-09-28'
--and chnl_cate_0='服务产业方案部'
;
...
from dmc_dev.btgyx_value_userpin_cnt where grouping__id=1 and dt = '2021-09-27';
...
from dmc_dev.btgyx_value_userpin
where substr(value_tm,1,10) between '2021-09-24' and '2021-09-28' and value_hlth_flag=1 and coalesce(value_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
...
,valueuser2--价值用户-2类-新达成-首贷当天结清但后续复购
,users_jz-valueuser1-valueuser2 valueuser3_1 --价值用户-年新-剔除新达成2类重合
from dmc_dev.btgyx_value_userpin_cnt_online where grouping__id=0 and chnl_online is not null and dt between '2021-09-27' and '2021-09-28';

--阿服首贷
select dt
,chnl_cate,users_fst+coalesce(users_fst_1,0)
from dmc_dev.btgyx_fst_userpin_cnt where grouping__id=0 and chnl_cate is not null and dt = '2021-09-28'
and chnl_cate='服务产业方案部';

--全量首贷
select dt
,users_fst
from dmc_dev.btgyx_fst_userpin_cnt where grouping__id=1 and dt = '2021-09-28';


--渠道值价值用户
select substr(value_tm,1,10) dt,actv_chnl_code_lv1,count(pin) users_jz,count(case when value_typ=1 then pin end) valueuser1,count(case when value_typ=2 then pin end) valueuser2
from dmc_dev.btgyx_value_userpin
where substr(value_tm,1,10) = '2021-09-28' and value_hlth_flag=1 and coalesce(value_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
and actv_chnl_code_lv1 in ('103003887040')
and chnl_cate_0='服务产业方案部'
and value_typ in (1,2)
group by 1,2;

--唤醒用户商户号
select substr(value_tm,1,10) dt,hx_sub_mht_no,count(pin) users_jz,count(case when value_typ=1 then pin end) valueuser1,count(case when value_typ=2 then pin end) valueuser2
from dmc_dev.btgyx_value_userpin
where substr(value_tm,1,10) = '2021-09-28' and value_hlth_flag=1 and coalesce(value_loan_user_age,0)<=55 and (fresh_bt_acct_flag=0 or coalesce(onl_or_ofl_code,'online')='online')
and (hx_sub_mht_no in ('110214978004','110214978001')
--or hx_biz_id in ('19')
)
and chnl_cate_0='服务产业方案部'
and value_typ in (3)
